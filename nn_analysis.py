...
#X = mcmc.sample_uHMC(np.random.random_sample(23) * 0.5, 
#                            1, 0.01, 0.5, auxiliary_gradient, 100)
...
### TEMPORARELY HERE - plotting potential
#d = 4
#for i in range(d):
#    plt.subplot(2, 2, i+1)
#    a,b,c,d,e,f = nn_potential.split_params(X[i])
...
# For debug: just perform the gradient and QUIT
onlyGradient = False #True
if (onlyGradient):
    time_for_one_gradient = time.time()
    print("Gradient descent on sample number 0")
    b2, b3, b4, W2, W3, W4 = nn_potential.split_params(X[0])
    if(nn_potential.performGradientDescent(nn_potential.x, nn_potential.y, 
                                        b2, b3, b4, W2, W3, W4, eta=10.5)[0]):
        print("MODE FOUND: sample number 0")
    time_for_one_gradient = time.time() - time_for_one_gradient
    print("*** EXPECTED TOTAL RUNNING TIME: ",
        str(datetime.timedelta(seconds =
            int(time_for_one_gradient * (len(X) - 1)))))

    for i in range(1, len(X)):
        print("Gradient descent on sample number", i)
        b2, b3, b4, W2, W3, W4 = nn_potential.split_params(X[i])
        if(nn_potential.performGradientDescent(nn_potential.x, nn_potential.y, 
                                        b2, b3, b4, W2, W3, W4, eta=10.5)[0]):
            print("MODE FOUND: sample number ", i)
    quit()
...
#        plt.bar(centroids[:,i], freq, color='magenta')
...
# Write to a file the list of the modes
